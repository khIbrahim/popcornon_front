...
import { useCinemaScreenings } from "../../hooks/useCinemas.ts";
...
export default function CinemaDrawer({
    ...
    const weekDays = useMemo(
        () => getNext7Days(weekStart),
        [weekStart]
    );

    const from = formatDateLocal(weekDays[0]);
    const to   = formatDateLocal(weekDays[6]);
    ...
    const groupedByMovie = useMemo(() => {
        ...
        screeningsForSelectedDate.forEach((screening) => {
            ...
            map.get(movieId)!.push(screening);
        });
